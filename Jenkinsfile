pipeline{
    ...
    stages{
        stage('Prepare'){
            steps {
                sh 'cd hobbyharvest && mvn clean'
            }
        }
        stage('Build') {
            steps {
                sh 'cd hobbyharvest && mvn install'
            }
        }
        stage('Test') {
            steps {
                sh 'cd hobbyharvest && mvn test'
            }
        }
            ...
            steps {
                script {
                   // 컨테이너의 이름
                    ...
                    sh "docker run -d -e TZ=Asia/Seoul -p 3131:8080 --name ${containerName} ${imageFullName}"
